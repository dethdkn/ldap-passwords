...
function extend(source: string, sizeRef: number): string{
  let extended = ''
  for(let i = 0; i < Math.floor(sizeRef / 64); i++) extended += source

  extended += source.slice(0, Math.max(0, sizeRef % 64))
  return extended
}
...
function rstr2binb(input: string): number[]{
  ...
  for(let i = 0; i < input.length * 8; i += 8) output[i >> 5] |= (input.codePointAt(i / 8) & 0xFF) << (24 - i % 32)
...
function binb2rstr(input: number[]): string{
  let output = ''
  for(let i = 0; i < input.length * 32; i += 8) output += String.fromCodePoint((input[i >> 5] >>> (24 - i % 32)) & 0xFF)
...
function sha512cryptIntermediate(password: string, salt: string){
  ...
  for(let cnt = password.length; cnt > 0; cnt >>= 1){
    if((cnt & 1) !== 0) intermediateInput += digest

    else intermediateInput += password
  }
...
function rstrSha512crypt(password: string, salt: string, rounds: number){
  ...
  let dpInput = ''
  for(let i = 0; i < password.length; i++) dpInput += password

  const dp = rstrSha512(dpInput)
  const p = extend(dp, password.length)
  let dsInput = ''
  for(let i = 0; i < (16 + digestA.codePointAt(0)); i++) dsInput += salt
...
function _(password: string, salt: string): string{
  ...
  for(let i = 0; i < input.length; i += 3){
    if(order[i + 1] === undefined){
      const char1 = input.codePointAt(order[i + 0]) & Number.parseInt('00111111', 2)
      const char2 = (input.codePointAt(order[i + 0]) & Number.parseInt('11000000', 2)) >>> 6
      output += tab.charAt(char1) + tab.charAt(char2)
    }
    else {
      const char1 = input.codePointAt(order[i + 0]) & Number.parseInt('00111111', 2)
      const char2 = (((input.codePointAt(order[i + 0]) & Number.parseInt('11000000', 2)) >>> 6) | (input.codePointAt(order[i + 1]) & Number.parseInt('00001111', 2)) << 2)
      const char3 = (((input.codePointAt(order[i + 1]) & Number.parseInt('11110000', 2)) >> 4) | (input.codePointAt(order[i + 2]) & Number.parseInt('00000011', 2)) << 4)
      const char4 = (input.codePointAt(order[i + 2]) & Number.parseInt('11111100', 2)) >>> 2
...
export function sha512Crypt(textPassword: string, salt = ''): string{
  const s = salt || randomBytes(16).toString('base64').slice(0, 16)
  if(s.length > 16) throw new Error('The maximum length of salt is 16 characters')
  return _(textPassword, s)
}
